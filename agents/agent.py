...
try:
    from rag.vectorstore import query_knowledge
    RAG_AVAILABLE = True
except Exception:
    RAG_AVAILABLE = False
    def query_knowledge(query: str, city: str = "", n: int = 5) -> str:
        return ""
...
- TOTAL TRANSPORT: ${transport_total} (${transport_pp}/person)"""
    ...
    rag_context = query_knowledge(
        f"budget travel student {destination} hotels food activities {state.get('vibe', '')}",
        city=destination, n=5,
    )
