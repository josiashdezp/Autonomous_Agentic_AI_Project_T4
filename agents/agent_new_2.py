...
# Secondary LLM — used exclusively as Judge
llm_judge = ChatOpenAI(model="gpt-4o-mini", temperature=0, openai_api_key=os.getenv("OPENAI_API_KEY"))
...
def mask_pii(text: str) -> tuple:
    """Redacts PII from text. Returns (sanitized_text, list_of_types_found)."""
    found = []
    for pii_type, (pattern, replacement) in _PII_PATTERNS.items():
        if pattern.search(text):
            text = pattern.sub(replacement, text)
            found.append(pii_type)
            logger.warning("PII guardrail: redacted %s", pii_type)
    return text, found
...
_SAFETY_PATTERNS = [
    re.compile(r"\b(cocaine|heroin|fentanyl|meth|narcotics?|drug\s+deal)\b", re.I),
    re.compile(r"\b(smuggl|traffick|launder)\b", re.I),
    re.compile(r"\b(kill|murder|assault|weapon|bomb|explosiv)\b", re.I),
    re.compile(r"\b(strip\s*club|brothel|prostitut|escort\s+service)\b", re.I),
    re.compile(r"ignore\s+(previous|prior|all|your)\s+(instructions?|rules?|prompts?)", re.I),
    re.compile(r"(jailbreak|dan\s+mode|developer\s+mode)", re.I),
    re.compile(r"(bypass|disregard|override).{0,20}(safety|guardrail|filter|rules?)", re.I),
    re.compile(r"forget\s+(your\s+)?(instructions?|rules?|training)", re.I),
]

_SAFETY_BLOCKED_MSG = "I can only help with US travel planning. Please keep questions travel-related."

def check_safety(text: str) -> tuple:
    """Checks input for harmful or injection content. Returns (is_blocked, message)."""
    for pattern in _SAFETY_PATTERNS:
        if pattern.search(text):
            logger.warning("Safety guardrail: blocked message")
            return True, _SAFETY_BLOCKED_MSG
    return False, ""
...
def parse_input_node(state: TravelState) -> dict:
    ...
    # Guardrails: safety check + PII masking - added
    _blocked, _block_msg = check_safety(latest_user_msg)
    if _blocked:
        return {"messages": [{"role": "assistant", "content": _block_msg}]}
    latest_user_msg, _pii_found = mask_pii(latest_user_msg)
    if _pii_found:
        logger.warning("PII redacted from user input: %s", _pii_found)
...
# LLM AS JUDGE
# ══════════════════════════════════════════════════════════════════════════════
@traceable(name="llm_judge_itinerary")
def llm_judge_itinerary(itinerary: str, state: dict) -> dict:
    """
    Secondary LLM evaluates the generated itinerary against the original
    user request (ground truth). Loops max 2 times per lecture exit strategy.
    """
    original_request = {
        "destination":        state.get("destination"),
        "num_days":           state.get("num_days"),
    ...
    prompt = f"""You are a strict travel itinerary evaluator. Assess whether the
generated itinerary meets the user's original request.

ORIGINAL USER REQUEST (ground truth):
- Destination: {original_request['destination']}
- Days: {original_request['num_days']}
- Budget: ${original_request['budget']} ({original_request['budget_type']})
- Travelers: {original_request['num_travelers']}
- Vibe: {original_request['vibe']}
- Accommodation: {original_request['accommodation_pref']}
...
1. CONTEXT PRECISION: Does itinerary match destination, vibe, accommodation?
2. TOOL CORRECTION: Is output valid? Has budget table, day sections, restaurant names?
3. COMPLETENESS: Covers all {original_request['num_days']} days with activities?
4. HALLUCINATION: Stays in {original_request['destination']}? No wrong cities?
...
{{
  ...
  "dimensions": {{
    "context_precision": "PASS" or "FAIL",
    "tool_correction": "PASS" or "FAIL",
    "completeness": "PASS" or "FAIL",
    "hallucination": "PASS" or "FAIL"
  }}
}}
PASS only if ALL 4 dimensions pass."""
    ...
    while status == "FAIL" and attempt < max_attempts:
        attempt += 1
        try:
            r = call_llm(llm_judge, [SystemMessage(content=prompt)])
            raw = extract_text(r).strip().replace("```json", "").replace("```", "")
...
3 money-saving tips for {destination}"""
    ...
    # LLM as Judge — secondary LLM evaluates the generated itinerary
    judge_result = llm_judge_itinerary(itinerary, state)
    return {
        "stage": "done",
        "itinerary": itinerary,
